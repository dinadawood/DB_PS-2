--------------------------------------------------
--------------------TRG_01------------------------
CREATE OR REPLACE TRIGGER trg01_customer BEFORE
    INSERT OR UPDATE ON customer
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.customer_crtd_id := user;
        :new.customer_crtd_dt := sysdate;
    END IF;

    :new.customer_updt_id := user;
    :new.customer_updt_dt := sysdate;
END;
/

--------------------------------------------------
CREATE OR REPLACE TRIGGER trg01_orders BEFORE
    INSERT OR UPDATE ON orders
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.orders_crtd_id := user;
        :new.orders_crtd_dt := sysdate;
    END IF;

    :new.orders_updt_id := user;
    :new.orders_updt_dt := sysdate;
END;
/

--------------------------------------------------
CREATE OR REPLACE TRIGGER trg01_orders_line BEFORE
    INSERT OR UPDATE ON orders_line
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.orders_line_crtd_id := user;
        :new.orders_line_crtd_dt := sysdate;
    END IF;

    :new.orders_line_updt_id := user;
    :new.orders_line_updt_dt := sysdate;
END;
/
...
--------------------------------------------------
--------------------TRG_02------------------------
CREATE OR REPLACE TRIGGER trg02_customer BEFORE
    INSERT OR UPDATE ON customer
    FOR EACH ROW
BEGIN
    IF
...
--------------------------------------------------
CREATE OR REPLACE TRIGGER trg02_orders BEFORE
    INSERT OR UPDATE ON orders
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.orders_id := sys_guid();
    ELSE
        :new.orders_id := :old.orders_id;
    END IF;
END;
/

--------------------------------------------------
CREATE OR REPLACE TRIGGER trg02_orders_line BEFORE
    INSERT OR UPDATE ON orders_line
    FOR EACH ROW
BEGIN
    IF
        inserting
        AND :new.orders_line_id IS NULL
    THEN
        :new.orders_line_id := sys_guid();
    ELSE
        :new.orders_line_id := :old.orders_line_id;
    END IF;
END;
/
...
BEGIN
    IF
        inserting
        AND :new.product_status_id IS NULL
    THEN
        :new.product_status_id := sys_guid();
    ELSE
        :new.product_status_id := :old.product_status_id;
    END IF;
END;
/
...
BEGIN
    nbr_cust_in := 100;
    pkg_data.create_customers(nbr_cust_in => nbr_cust_in);
--rollback; 
END;
/
...
BEGIN
    nbr_cust_in := 900;
    pkg_data.create_orders(nbr_cust_in => nbr_cust_in);
--rollback; 
END;
/
...
--------------------------------------------------
--------------------TRG01-------------------------
CREATE OR REPLACE TRIGGER trg01_gender BEFORE
    INSERT OR UPDATE ON gender
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.gender_crtd_id := user;
        :new.gender_crtd_dt := sysdate;
    END IF;

    :new.gender_updt_id := user;
    :new.gender_updt_dt := sysdate;
END;
/

--------------------------------------------------
--------------------TRG02-------------------------
CREATE OR REPLACE TRIGGER trg02_gender BEFORE
    INSERT OR UPDATE ON gender
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.gender_id := sys_guid();
    ELSE
        :new.gender_id := :old.gender_id;
    END IF;
END;
/
...
WHERE
    gender_name = 'Transgendr';

commit;

--------------------------------------------------
--------------------TRG01-------------------------
CREATE OR REPLACE TRIGGER trg01_product_price BEFORE
    INSERT OR UPDATE ON product_price
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.product_price_crtd_id := user;
        :new.product_price_crtd_dt := sysdate;
    END IF;

    :new.product_price_updt_id := user;
    :new.product_price_updt_dt := sysdate;
END;
/

--------------------------------------------------
--------------------TRG02-------------------------
CREATE OR REPLACE TRIGGER trg02_product_price BEFORE
    INSERT OR UPDATE ON product_price
    FOR EACH ROW
BEGIN
    IF inserting THEN
        :new.product_price_id := sys_guid();
    ELSE
        :new.product_price_id := :old.product_price_id;
    END IF;
END;
/
